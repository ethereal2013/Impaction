...
{
    return ImGui::GetCurrentContext() ? static_cast<ImGui_ImplOpenGL3_Data*>(ImGui::GetIO().BackendRendererUserData) : nullptr;
}
...
static void ImGui_ImplOpenGL3_SetupRenderState(ImDrawData* draw_data, ImGui_ImplOpenGL3_RenderState* render_state, int fb_width, int fb_height, GLuint vertex_array_object)
{
    ...
    if (bd->HasClipOrigin)
    {
        GLenum current_clip_origin = 0; glGetIntegerv(GL_CLIP_ORIGIN, reinterpret_cast<GLint*>(&current_clip_origin));
    ...
    static_cast<void>(vertex_array_object);
    ...
    GL_CALL(glVertexAttribPointer(bd->AttribLocationVtxPos,   2, GL_FLOAT,         GL_FALSE, sizeof(ImDrawVert), reinterpret_cast<GLvoid*>(offsetof(ImDrawVert, pos))));
    GL_CALL(glVertexAttribPointer(bd->AttribLocationVtxUV,    2, GL_FLOAT,         GL_FALSE, sizeof(ImDrawVert), reinterpret_cast<GLvoid*>(offsetof(ImDrawVert, uv))));
    GL_CALL(glVertexAttribPointer(bd->AttribLocationVtxColor, 4, GL_UNSIGNED_BYTE, GL_TRUE, sizeof(ImDrawVert), reinterpret_cast<GLvoid*>(offsetof(ImDrawVert, col))));
}
...
void    ImGui_ImplOpenGL3_RenderDrawData(ImDrawData* draw_data)
{
    ...
    // Backup GL state
    GLenum last_active_texture; glGetIntegerv(GL_ACTIVE_TEXTURE, reinterpret_cast<GLint*>(&last_active_texture));
    glActiveTexture(GL_TEXTURE0);
    GLuint last_program; glGetIntegerv(GL_CURRENT_PROGRAM, reinterpret_cast<GLint*>(&last_program));
    GLuint last_texture; glGetIntegerv(GL_TEXTURE_BINDING_2D, reinterpret_cast<GLint*>(&last_texture));
#ifdef IMGUI_IMPL_OPENGL_MAY_HAVE_BIND_SAMPLER
    GLuint last_sampler; if (bd->HasBindSampler) { glGetIntegerv(GL_SAMPLER_BINDING, reinterpret_cast<GLint*>(&last_sampler)); } else { last_sampler = 0; }
#endif
    GLuint last_array_buffer; glGetIntegerv(GL_ARRAY_BUFFER_BINDING, reinterpret_cast<GLint*>(&last_array_buffer));
#ifndef IMGUI_IMPL_OPENGL_USE_VERTEX_ARRAY
...
#endif
#ifdef IMGUI_IMPL_OPENGL_USE_VERTEX_ARRAY
    GLuint last_vertex_array_object; glGetIntegerv(GL_VERTEX_ARRAY_BINDING, reinterpret_cast<GLint*>(&last_vertex_array_object));
#endif
#ifdef IMGUI_IMPL_OPENGL_MAY_HAVE_POLYGON_MODE
    ...
    GLenum last_blend_equation_rgb; glGetIntegerv(GL_BLEND_EQUATION_RGB, reinterpret_cast<GLint*>(&last_blend_equation_rgb));
    GLenum last_blend_equation_alpha; glGetIntegerv(GL_BLEND_EQUATION_ALPHA, reinterpret_cast<GLint*>(&last_blend_equation_alpha));
    ...
    for (const ImDrawList* draw_list : draw_data->CmdLists)
    {
        ...
        const GLsizeiptr vtx_buffer_size = static_cast<GLsizeiptr>(draw_list->VtxBuffer.Size) * static_cast<GLsizeiptr>(sizeof(ImDrawVert));
        const GLsizeiptr idx_buffer_size = static_cast<GLsizeiptr>(draw_list->IdxBuffer.Size) * static_cast<GLsizeiptr>(sizeof(ImDrawIdx));
        if (bd->UseBufferSubData)
        {
            ...
            GL_CALL(glBufferSubData(GL_ARRAY_BUFFER, 0, vtx_buffer_size, static_cast<const GLvoid*>(draw_list->VtxBuffer.Data)));
            GL_CALL(glBufferSubData(GL_ELEMENT_ARRAY_BUFFER, 0, idx_buffer_size, static_cast<const GLvoid*>(draw_list->IdxBuffer.Data)));
        }
        else
        {
            GL_CALL(glBufferData(GL_ARRAY_BUFFER, vtx_buffer_size, static_cast<const GLvoid*>(draw_list->VtxBuffer.Data), GL_STREAM_DRAW));
            GL_CALL(glBufferData(GL_ELEMENT_ARRAY_BUFFER, idx_buffer_size, static_cast<const GLvoid*>(draw_list->IdxBuffer.Data), GL_STREAM_DRAW));
        }
        ...
        for (int cmd_i = 0; cmd_i < draw_list->CmdBuffer.Size; cmd_i++)
        {
            ...
            else
            {
                ...
                GL_CALL(glScissor(static_cast<int>(clip_min.x), static_cast<int>(static_cast<float>(fb_height) - clip_max.y), static_cast<int>(clip_max.x - clip_min.x), static_cast<int>(clip_max.y - clip_min.y)));

                // Bind texture, Draw
                GL_CALL(glBindTexture(GL_TEXTURE_2D, static_cast<GLuint>(pcmd->GetTexID())));
                ...
                if (bd->GlVersion >= 320)
                    GL_CALL(glDrawElementsBaseVertex(GL_TRIANGLES, static_cast<GLsizei>(pcmd->ElemCount), sizeof(ImDrawIdx) == 2 ? GL_UNSIGNED_SHORT : GL_UNSIGNED_INT, reinterpret_cast<void*>(static_cast<intptr_t>(pcmd->IdxOffset * sizeof(ImDrawIdx))), static_cast<GLint>(pcmd->VtxOffset)));
                else
#endif
                GL_CALL(glDrawElements(GL_TRIANGLES, static_cast<GLsizei>(pcmd->ElemCount), sizeof(ImDrawIdx) == 2 ? GL_UNSIGNED_SHORT : GL_UNSIGNED_INT, reinterpret_cast<void*>(static_cast<intptr_t>(pcmd->IdxOffset * sizeof(ImDrawIdx)))));
            }
        }
    }
    ...
    if (bd->HasPolygonMode) { if (bd->GlVersion <= 310 || bd->GlProfileIsCompat) { glPolygonMode(GL_FRONT, static_cast<GLenum>(last_polygon_mode[0])); glPolygonMode(GL_BACK, static_cast<GLenum>(last_polygon_mode[1])); } else { glPolygonMode(GL_FRONT_AND_BACK, static_cast<GLenum>(last_polygon_mode[0])); } }
    ...
    static_cast<void>(bd); // Not all compilation paths use this
}
...
void ImGui_ImplOpenGL3_UpdateTexture(ImTextureData* tex)
{
    ...
    GLint last_unpack_row_length = 0; static_cast<void>(last_unpack_row_length);
    GLint last_unpack_alignment = 0; static_cast<void>(last_unpack_alignment);
    ...
    if (tex->Status == ImTextureStatus_WantCreate)
    {
        ...
        // Store identifiers
        tex->SetTexID(static_cast<ImTextureID>(gl_texture_id));
...
static bool CheckShader(GLuint handle, const char* desc)
{
    ...
    if (log_length > 1)
    {
        ...
        glGetShaderInfoLog(handle, log_length, nullptr, reinterpret_cast<GLchar*>(buf.begin()));
        fprintf(stderr, "%s\n", buf.begin());
    }
...
static bool CheckProgram(GLuint handle, const char* desc)
{
    ...
    if (log_length > 1)
    {
        ImVector<char> buf;
        buf.resize(static_cast<int>(log_length + 1));
        glGetProgramInfoLog(handle, log_length, nullptr, reinterpret_cast<GLchar*>(buf.begin()));
        fprintf(stderr, "%s\n", buf.begin());
    }
    return static_cast<GLboolean>(status) == GL_TRUE;
}
...
bool    ImGui_ImplOpenGL3_Init(const char* glsl_version)
{
    ...
    */
...
#ifdef IMGUI_IMPL_OPENGL_DEBUG
    printf("GlVersion = %d, \"%s\"\nGlProfileIsCompat = %d\nGlProfileMask = 0x%X\nGlProfileIsES2/IsEs3 = %d/%d\nGL_VENDOR = '%s'\nGL_RENDERER = '%s'\n", bd->GlVersion, gl_version_str, bd->GlProfileIsCompat, bd->GlProfileMask, bd->GlProfileIsES2, bd->GlProfileIsES3, reinterpret_cast<const char*>(glGetString(GL_VENDOR)), reinterpret_cast<const char*>(glGetString(GL_RENDERER))); // [DEBUG]
#endif
    ...
    ImGuiPlatformIO& platform_io = ImGui::GetPlatformIO();
    platform_io.Renderer_TextureMaxWidth = platform_io.Renderer_TextureMaxHeight = static_cast<int>(bd->MaxTextureSize);
    ...
    IM_ASSERT(static_cast<int>(strlen(glsl_version)) + 2 < IM_COUNTOF(bd->GlslVersionString));
